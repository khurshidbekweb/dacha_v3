...
const TypeCottage = ({ paramsId }: placeProps) => {
    const { data: cottages, isLoading } = ALL_DATA.useCottageByType(paramsId)
    const { t } = useTranslation()
    const type = cottages?.length && cottages[0].cottageType[0].name || ''
    return (
        <>
            <Navbar />
